...
import { ContractJson } from '../helpers/types/hardhat-type-extensions';
...
const publishContract = (contractName: string, networkName: string) => {
  try {
    ...
    const contractJson: ContractJson = JSON.parse(contract);
    ...
    console.log(' 📠 Published ' + chalk.green(contractName) + ' hardhat_contracts.json');
